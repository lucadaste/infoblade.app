...
export async function buildContextGraph(supabase, { tickers = [], category = null } = {}) {
  const { data: preds, error } = await supabase
    .from('predictions')
    .select('id, winner_tickers, loser_tickers, correct, category, analysis, created_at, topic, source_types, contrarian, coverage_volume_bucket')
    .not('correct', 'is', null)
  ...
  // ── Source-type reward loop: per-category accuracy by cited source type ────
  // { [sourceType]: { overall:{total,correct}, contrarian:{total,correct} } }
  // Gated the same way categoryAccuracy is (needs `category` + a real sample) so
  // this stays silent — not just sparse — before the category has its own track
  // record. See sentiment/app/whitelist.py's WHITELIST_MIN_CALLS for the same
  // cold-start pattern applied at the StockTwits-account level.
  let sourceTypeAccuracy = null;
  if (category && categoryAccuracy && categoryAccuracy.total >= 20) {
    const acc = {};
    for (const p of preds) {
      if (p.category !== category || !p.source_types) continue;
      for (const t of Object.keys(p.source_types)) {
        acc[t] ??= { overall: { total: 0, correct: 0 }, contrarian: { total: 0, correct: 0 } };
        acc[t].overall.total++;
        if (p.correct) acc[t].overall.correct++;
        if (p.contrarian) {
          acc[t].contrarian.total++;
          if (p.correct) acc[t].contrarian.correct++;
        }
      }
    }
    if (Object.keys(acc).length) sourceTypeAccuracy = acc;
  }

  // ── Coverage-volume reward loop: per-category accuracy by how heavily-covered ──
  // the cited story was — { low: {total,correct}, medium: {...}, high: {...} }.
  // Same gating as sourceTypeAccuracy; answers empirically whether leaning on
  // widely-covered/consensus stories helps or hurts per category, rather than
  // assuming heavy coverage means either "well-confirmed" or "already priced in."
  let coverageVolumeAccuracy = null;
  if (category && categoryAccuracy && categoryAccuracy.total >= 20) {
    const acc = { low: { total: 0, correct: 0 }, medium: { total: 0, correct: 0 }, high: { total: 0, correct: 0 } };
    let any = false;
    for (const p of preds) {
      if (p.category !== category || !p.coverage_volume_bucket) continue;
      const bucket = acc[p.coverage_volume_bucket];
      if (!bucket) continue;
      any = true;
      bucket.total++;
      if (p.correct) bucket.correct++;
    }
    if (any) coverageVolumeAccuracy = acc;
  }
...
export function formatContextForPrompt(graph) {
  ...
  // Source-type reliability (prediction-markets reward loop) — silent until a
  // (category, source-type, bucket) cell clears SOURCE_TYPE_MIN_CALLS, so early
  // sparse data doesn't masquerade as a real signal. Prefers the contrarian
  // bucket over overall when it alone qualifies — the user's ask was specifically
  // "especially when going against the market."
  if (graph.sourceTypeAccuracy) {
    const minCalls = parseInt(process.env.SOURCE_TYPE_MIN_CALLS, 10) || 15;
    const parts = [];
    for (const [type, s] of Object.entries(graph.sourceTypeAccuracy)) {
      const useContrarian = s.contrarian.total >= minCalls;
      const useOverall = !useContrarian && s.overall.total >= minCalls;
      if (!useContrarian && !useOverall) continue;
      const bucket = useContrarian ? s.contrarian : s.overall;
      const pct = Math.round(bucket.correct / bucket.total * 100);
      parts.push(`${type}: ${pct}% accurate${useContrarian ? ' on contrarian calls' : ''} (${bucket.correct}/${bucket.total})`);
    }
    if (parts.length) lines.push(`\nSource-type reliability (${graph.categoryAccuracy?.category || 'this category'}): ${parts.join(' | ')}`);
  }

  // Coverage-volume reliability — same min-sample gating, tells the model whether
  // heavily-covered stories have actually been a good or bad signal to lean on
  // historically in this category, instead of leaving that ambiguous every time.
  if (graph.coverageVolumeAccuracy) {
    const minCalls = parseInt(process.env.SOURCE_TYPE_MIN_CALLS, 10) || 15;
    const parts = [];
    for (const bucket of ['low', 'medium', 'high']) {
      const s = graph.coverageVolumeAccuracy[bucket];
      if (!s || s.total < minCalls) continue;
      const pct = Math.round(s.correct / s.total * 100);
      parts.push(`${bucket} coverage: ${pct}% accurate (${s.correct}/${s.total})`);
    }
    if (parts.length) lines.push(`\nCoverage-volume reliability (${graph.categoryAccuracy?.category || 'this category'}): ${parts.join(' | ')}`);
  }
